// Shared by both directions of profile Import/Export (lib/importExport.js):
...
// Must match lib/qaHistory.js's own MAX_ENTRIES exactly (both cap the same
// qaHistory list at 50). Duplicated as a literal rather than imported: the
// server-era version of this file imported the constant from
// routes/qa-history.js, but that module pulled in express + better-sqlite3
// transitively — dragging Node-only deps into this otherwise-pure validator
// and into the Vite bundle. Two definitions of "50" is a smaller risk than
// that import.
...
// Identity memory travels as a flat { canonicalKey: value } object. Lenient
// like qaHistory: drop unknown keys (not in the registry) and non-string values rather
