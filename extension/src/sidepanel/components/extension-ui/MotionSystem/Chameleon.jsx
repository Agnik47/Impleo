...
/*
 * Ported verbatim from landing/src/assets/mascot/Chameleon.jsx — same
 * geometry, same six pose states, same brand hex constants. Not a new mascot:
 * the extension previously used a static PNG (HeroExtentionImg.png) for the
 * hero mascot, which can't breathe or blink. This SVG component already does
 * both (see the eyelid `animate-blink` below), so reusing it here — rather
 * than inventing new mascot art — is what makes the extension's hero feel
 * like the same character as the landing page, not a lookalike.
 *
 * Duplicated rather than imported cross-package: the extension and landing
 * are separate Vite apps with no shared-package setup, so this file and its
 * source stay in sync by hand. If the mascot's design changes, update both.
 *
