...
> *"Read INSTALLATION.md in the repo root and set up this project locally — install dependencies and build the extension."*

<br />

## What gets installed

Impleo is a single local piece — a browser extension. There is no server to run.
...
There is no hosted backend, no local server, and no `.env` file to configure. The extension runs entirely inside your browser; your AI provider API key is entered once through the extension's onboarding/Settings UI and stored in your browser's local extension storage.
...
Node is needed only to **build** the extension (Vite/React/Tailwind). If Node is missing or below v18, install it from [nodejs.org](https://nodejs.org/) (LTS release) before continuing. No native modules are compiled — the build is pure JavaScript tooling.
...
If you were invoked from inside an existing clone, skip this step and confirm you're at the repository root (a directory containing `extension/`, `landing/`, and this file).
...
Expected result: `node_modules/` created for the extension's Vite/React/Tailwind toolchain, no `npm error` output.
...
This runs Vite and produces `extension/dist/` — the folder Chrome loads as an unpacked extension. Confirm `extension/dist/manifest.json` exists after this step. That is the entire build; there is nothing else to start or keep running.

### Step 5 — Load the unpacked extension (requires a human)
...
4. Select the `extension/dist` folder produced in Step 4.
5. Pin the Impleo icon to the toolbar for easy access (optional).

### Step 6 — Configure an API key (requires a human)
...
The key is stored in your browser's local extension storage and is sent only to the provider you chose, as that request's authentication header. It never goes to any Impleo backend, because there isn't one.
...
Then load `extension/dist` as an unpacked extension via `chrome://extensions` and configure your API key through the side panel's Settings. No server, no background process to keep alive.
...
| Run the landing page locally (optional) | `cd landing && npm install && npm run dev` | Unrelated to the extension — marketing site only |
...
| Extract or Generate errors in the side panel | Provider request failed (bad key, rate limit, or network) | Open the side panel devtools console for the exact provider error; confirm the key still works in the provider's dashboard |
| `npm install` or `npm run build` fails | Node below v18, or a stale `node_modules/` | Confirm `node -v` ≥ 18; delete `extension/node_modules` and re-run `npm install` |
...
│   └── src/sidepanel/     React side panel + lib/ (prompt builder, provider adapters,
│                          storage) — the whole app, no server
